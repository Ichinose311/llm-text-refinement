...
def create_recommend_data(data, llm):
    ...
    for candidate_info in candidates_info_list:
        # 対話要約 + アイテム情報から、ユーザ向け推薦理由を生成
        recommend_sentence = llm.generate_rec_reason_with_dialogue(
            dialogue_summary,
            candidate_info,
            temperature=0.2
        )
    ...
    recommend_data = {"predicted_score": predicted_scores, "score": scores}
    return create_formatted_data(recommend_data)
...
def main():
    ...
    output_dir = os.path.join(BASE_DIR, "../../data/Tabidachi/recommend_data_reason_v16_test/")
    # 出力ディレクトリが存在しない場合は作成
